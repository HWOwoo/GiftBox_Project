		...
		<table class="goods_table">
			...
			<tbody>
				<c:forEach items="${orderList}" var="ol">
					<tr>
						<td>
							<div class="image_wrap" data-giftid="${ol.imageList[0].giftId}" data-path="${ol.imageList[0].uploadPath}" data-uuid="${ol.imageList[0].uuid}" data-filename="${ol.imageList[0].fileName}">
								<img>
							</div>
						</td>
						<td>${ol.giftName}</td>
						<td class="goods_table_price_td">
							<fmt:formatNumber value="${ol.salePrice}" pattern="#,### 원" /> | 수량 ${ol.giftCount}개
							<br><fmt:formatNumber value="${ol.totalPrice}" pattern="#,### 원" />
							<br>[<fmt:formatNumber value="${ol.totalPoint}" pattern="#,### 원" />P]
							<input type="hidden" class="individual_giftPrice_input" value="${ol.giftPrice}">
							<input type="hidden" class="individual_salePrice_input" value="${ol.salePrice}">
							<input type="hidden" class="individual_giftCount_input" value="${ol.giftCount}">
							<input type="hidden" class="individual_totalPrice_input" value="${ol.salePrice * ol.giftCount}">
							...
							<input type="hidden" class="individual_giftId_input" value="${ol.giftId}">
						</td>
					</tr>							
				</c:forEach>
...
<script>
	...
	$(".order_btn").on("click", function(){
		...
		$(".goods_table_price_td").each(function(index, element){
			...
			let giftId_input = "<input name='orders[" + index + "].giftId' type='hidden' value='" + giftId + "'>";
			form_contents += giftId_input;
			let giftCount_input = "<input name='orders[" + index + "].giftCount' type='hidden' value='" + giftCount + "'>";
